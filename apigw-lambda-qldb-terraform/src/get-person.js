...
module.exports.handler = async (event) => {
  ...
  console.log(`In the get person handler with: personid ${personid}`);
  ...
  try {
    ...
    await qldbDriver.executeLambda(async (txn) => {
        ...
        // Insert an order doc
        const statement = 'SELECT * FROM Person WHERE personId = ?'
        ...
        if (resultList.length === 0) {
          ...
          person = JSON.parse(JSON.stringify(resultList[0]));
        }
