...
class ApigwRestEventBridgeSQSStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        ### First SQS Queue ###
        first_destination_queue = sqs.Queue(
            scope=self,
            id= "FirstDestinationQueue"
        )

        ### Second SQS Queue ###
        second_destination_queue = sqs.Queue(
            scope=self,
            id= "SecondDestinationQueue"
        )
        ...
        event_bus = events.EventBus(
            scope=self,
            id= "EventBus",
            event_bus_name="fanout-bus"

        )

        ### Event Rule that sends events to SQS when rule pattern matches ###
        event_rule = events.Rule(
            ...
            targets=[
                targets.SqsQueue(first_destination_queue),
                targets.SqsQueue(second_destination_queue)
            ]
        )
