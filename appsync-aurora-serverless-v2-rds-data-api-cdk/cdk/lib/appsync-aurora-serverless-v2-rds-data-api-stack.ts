    ...
    dataSource.createResolver('ListTodosResolver', {
      ...
      code: appsync.Code.fromInline(`
        ...
        export function request(ctx) {
          return sql\`SELECT id, title, completed FROM todos ORDER BY id\`;
        }
        export function response(ctx) {
          if (ctx.error) {
            util.error(ctx.error.message, ctx.error.type);
          }
          const rows = ctx.result.sqlStatementResults[0].records ?? [];
          return rows.map((r) => ({
            id: r[0].longValue,
            title: r[1].stringValue,
            completed: r[2].booleanValue,
          }));
        }
      `),
    });
      ...
      code: appsync.Code.fromInline(`
        ...
        export function request(ctx) {
          ...
          return createPgStatement(
            sql\`INSERT INTO todos (title, completed) VALUES (:title, false) RETURNING id, title, completed\`
          )({ title });
        }
        export function response(ctx) {
          if (ctx.error) {
