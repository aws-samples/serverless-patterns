...
const shouldRunIntegration = existsSync(cdkOutputsPath);
// Keep the suite runnable in fresh checkouts where the stack has not been
// deployed yet; deployment-specific coverage still runs automatically once the
// generated outputs file is present.
const describeWhenDeployed = shouldRunIntegration ? describe : describe.skip;

if (!shouldRunIntegration) {
  console.warn(
    `Skipping integration tests because ${cdkOutputsPath} is missing. Deploy the stack to run them.`
  );
}

describeWhenDeployed(
  "Bedrock Model invoked with Programmatic Cognito Auth (Lambda Resolver)",
  () => {
    beforeAll(async () => {
      let cdkOutputs;
      try {
        const cdkOutputsFile = readFileSync(cdkOutputsPath);
      ...
      try {
        ...
        console.log(
          `Test user ${TEST_USER_EMAIL} logged in. ID token obtained.`
        );
      } catch (error) {
      ...
      if (TEST_USER_EMAIL && USER_POOL_ID && cognitoClient) {
        ...
        try {
          ...
          if (error.name === "UserNotFoundException") {
            ...
            console.error(
              `Failed to delete test user ${TEST_USER_EMAIL}:`,
              error
            );
          }
        }
    ...
    test(
      "The user performs the invoke mutation using a programmatically obtained Cognito ID token",
      async () => {
        if (!idToken) {
          throw new Error(
        ...
        console.log("Answer:", mutationPromise.data?.invoke ?? "<no answer>");
