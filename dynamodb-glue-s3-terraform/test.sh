...
aws dynamodb put-item --region $REGION --table-name $TABLE_NAME --item '{"id":{"S":"test1"},"name":{"S":"John"},"age":{"N":"30"}}'
aws dynamodb put-item --region $REGION --table-name $TABLE_NAME --item '{"id":{"S":"test2"},"name":{"S":"Jane"},"age":{"N":"25"}}'
aws dynamodb put-item --region $REGION --table-name $TABLE_NAME --item '{"id":{"S":"test3"},"name":{"S":"Julie"},"age":{"N":"35"}}'

# Run the Glue job
echo "Starting Glue job..."
JOB_RUN_ID=$(aws glue start-job-run --region $REGION --job-name $JOB_NAME --query 'JobRunId' --output text)

echo "✅ Job started with ID: $JOB_RUN_ID"
echo ""
echo "Run this command to monitor job status:"
echo "  aws glue get-job-run --region $REGION --job-name $JOB_NAME --run-id $JOB_RUN_ID"
echo ""
echo "Run this command to check the results in S3:"
echo "  aws s3 ls --region $REGION s3://$BUCKET_NAME/data/ --recursive"
