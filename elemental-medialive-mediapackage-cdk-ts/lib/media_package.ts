  ...
  constructor(scope: Construct, id: string) {
    ...
    if(config.hls){
      ...
      const hlsPackage: mediapackage.CfnOriginEndpoint.HlsPackageProperty = {
        segmentDurationSeconds:
          config.hls_segment_duration_seconds,
        playlistWindowSeconds:
        config.hls_playlist_window_seconds,
        streamSelection: {
          minVideoBitsPerSecond:
          config.hls_min_video_bits_per_second,
          maxVideoBitsPerSecond:
          config.hls_max_video_bits_per_second,
    ...
    if(config.dash){
      const dashPackage: mediapackage.CfnOriginEndpoint.DashPackageProperty = {
        segmentDurationSeconds:
        config.dash_segment_duration_seconds,
        manifestWindowSeconds:
        config.dash_manifest_window_seconds,
        streamSelection: {
          minVideoBitsPerSecond:
          config.dash_min_video_bits_per_second,
          maxVideoBitsPerSecond:
          config.dash_max_video_bits_per_second,
    ...
    if(config.mss){
      const mssPackage: mediapackage.CfnOriginEndpoint.MssPackageProperty = {
        segmentDurationSeconds:
        config.mss_segment_duration_seconds,
        manifestWindowSeconds:
        config.mss_manifest_window_seconds,
        streamSelection: {
          minVideoBitsPerSecond:
          config.mss_min_video_bits_per_second,
          maxVideoBitsPerSecond:
          config.mss_max_video_bits_per_second,
