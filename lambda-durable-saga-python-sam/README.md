...
1. Set a shell variable to the stack name you chose — all testing commands below use it:
   ```bash
...
1. **Lambda durable function (Saga Orchestrator)**: A Python 3.13 Lambda function that orchestrates three checkpointed steps in sequence and executes compensating transactions on failure.
...
2. Test happy path (successful order):
   ...
   ```bash
   aws lambda invoke \
     --function-name "${STACK_NAME}-orchestrator:\$LATEST" \
...
This pattern is a reference implementation. Before using in production:

- **Enable Point-in-Time Recovery (PITR)** on all three DynamoDB tables (`PointInTimeRecoverySpecification: PointInTimeRecoveryEnabled: true`) to allow recovery from accidental writes or deletes.
- **Use a customer-managed KMS key (CMK)** for the payments table (`SSESpecification`) — it stores `customer_id` and payment amounts.
- **Tighten IAM permissions** — `DynamoDBCrudPolicy` includes `DeleteItem`, `Scan`, and `Query`, which this function never calls. For production, replace with an explicit action list: `dynamodb:GetItem`, `dynamodb:PutItem`, `dynamodb:UpdateItem`, `dynamodb:ConditionCheckItem`.
- **Monitor the compensation DLQ** — messages landing there mean a saga ended in an inconsistent state and require manual remediation.
