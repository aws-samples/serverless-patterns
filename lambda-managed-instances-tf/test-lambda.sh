...
# Test script for Hello World Lambda function on Managed Instances (Terraform)
...
aws lambda get-capacity-provider --capacity-provider-name lambda-capacity-provider-tf --query 'CapacityProvider.[CapacityProviderArn,State,InstanceRequirements.Architectures[0],CapacityProviderScalingConfig.ScalingMode]' --output table --profile "$PROFILE"

echo -e "${YELLOW}EC2 Instances provisioned for Lambda Managed Instances:${NC}"
# Get subnet IDs from capacity provider
SUBNET_IDS=$(aws lambda get-capacity-provider --capacity-provider-name lambda-capacity-provider-tf --query 'CapacityProvider.VpcConfig.SubnetIds' --output text --profile "$PROFILE" | tr '\t' ',')
SECURITY_GROUP_ID=$(aws lambda get-capacity-provider --capacity-provider-name lambda-capacity-provider-tf --query 'CapacityProvider.VpcConfig.SecurityGroupIds[0]' --output text --profile "$PROFILE")

# List EC2 instances tagged with this capacity provider
CAPACITY_PROVIDER_ARN=$(aws lambda get-capacity-provider --capacity-provider-name lambda-capacity-provider-tf --query 'CapacityProvider.CapacityProviderArn' --output text --profile "$PROFILE")
...
echo "   echo '{\"name\":\"Your Name\"}' | aws lambda invoke --function-name $FUNCTION_NAME:live --payload file:///dev/stdin --cli-binary-format raw-in-base64-out --profile $PROFILE output.json"
echo ""
echo "5. View capacity provider details:"
echo "   aws lambda get-capacity-provider --capacity-provider-name lambda-capacity-provider-tf --profile $PROFILE"
echo ""
echo "6. List EC2 instances for managed instances:"
echo "   aws ec2 describe-instances --filters \"Name=tag:aws:lambda:capacity-provider,Values=$CAPACITY_PROVIDER_ARN\" --profile $PROFILE"
echo ""
echo "7. View Terraform outputs:"
