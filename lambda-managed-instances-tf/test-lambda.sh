...
# Get capacity provider ARN from Terraform output or construct it
REGION=$(aws configure get region --profile "$PROFILE")
ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text --profile "$PROFILE")
CAPACITY_PROVIDER_ARN="arn:aws:lambda:${REGION}:${ACCOUNT_ID}:capacity-provider:lambda-capacity-provider"
