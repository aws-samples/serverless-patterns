# AWS Lambda MicroVM Sandboxed Code Execution

This pattern deploys a Lambda MicroVM that accepts Python code via HTTP and executes it in an isolated Firecracker VM. Each MicroVM provides hardware-level isolation, sub-second startup via snapshots, and automatic suspend/resume.

Learn more about this pattern at Serverless Land Patterns: https://serverlessland.com/patterns/lambda-microvm-code-execution-sandbox
...
2. The MicroVM runs from the Firecracker snapshot. It exposes an HTTP endpoint on port 8080.
3. Client sends Python code via `POST /execute` → the MicroVM executes it in a subprocess with a 30-second timeout → returns stdout/stderr/exit_code.
4. After 15 minutes of inactivity the MicroVM suspends. It auto-resumes on next request.
...
| `/run` | After MicroVM starts from snapshot | Delivers `microvmId`, reset unique state |
