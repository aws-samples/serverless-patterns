...
- Amazon Bedrock **model access for Anthropic Claude** enabled in your account (submit the use case form once from the Bedrock console). Claude Sonnet 4 must be available via the cross-region inference profile.
...
2. Inside the shell, run Claude Code:
   ...
   The `claude -p` call returns a completion generated by Claude Sonnet 4 on Amazon Bedrock.
