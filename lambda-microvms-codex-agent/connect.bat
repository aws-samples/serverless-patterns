@echo off
REM connect.bat -- Open an interactive shell into the Codex CLI AWS Lambda MicroVM (Windows)
REM
REM Usage: connect.bat <microvm-id> [region]
REM
REM Delegates to connect.sh via Git Bash for proper terminal handling.
REM Requires: Git Bash, websocat, AWS CLI v2
REM
...
if "%~1"=="" (
    echo Usage: %~nx0 ^<microvm-id^> [region]
    exit /b 1
)
