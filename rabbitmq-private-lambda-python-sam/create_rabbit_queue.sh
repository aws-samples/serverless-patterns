...
curl -i -u "$RABBITMQ_ADMIN_USER:$RABBITMQ_PASSWORD" -X PUT "$rabbitmq_https_broker_endpoint/api/vhosts/RABBITMQ_VIRTUAL_HOST"

curl -i -u "$RABBITMQ_ADMIN_USER:$RABBITMQ_PASSWORD" -X PUT -H "Content-type: application/json" -d '{"type": "fanout", "durable": true, "auto_delete": false, "internal": false}' "$rabbitmq_https_broker_endpoint/api/exchanges/RABBITMQ_VIRTUAL_HOST/RABBITMQ_EXCHANGE"

curl -i -u "$RABBITMQ_ADMIN_USER:$RABBITMQ_PASSWORD" -X PUT -H "Content-type: application/json" -d '{"durable": true, "auto_delete": false}' "$rabbitmq_https_broker_endpoint/api/queues/RABBITMQ_VIRTUAL_HOST/RABBITMQ_QUEUE_NAME"

curl -i -u "$RABBITMQ_ADMIN_USER:$RABBITMQ_PASSWORD" -X POST -H "Content-type: application/json" -d '{"routing_key": "RABBITMQ_EXCHANGE-RABBITMQ_QUEUE_NAME"}' "$rabbitmq_https_broker_endpoint/api/bindings/RABBITMQ_VIRTUAL_HOST/e/RABBITMQ_EXCHANGE/q/RABBITMQ_QUEUE_NAME"
