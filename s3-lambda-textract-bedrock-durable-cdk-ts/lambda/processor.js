const { withDurableExecution, createWaitStrategy } = require('@aws/durable-execution-sdk-js');
...
exports.handler = withDurableExecution(async (event, context) => {
  ...
  const jobId = await context.step('start-textract', async () => {
    const response = await textractClient.send(new StartDocumentTextDetectionCommand({
      DocumentLocation: {
  ...
  if (textractResult.status !== 'SUCCEEDED') {
    throw new Error(`Textract job failed with status: ${textractResult.status}`);
  }
