...
//   try {
//     const examplePatternData = fs.readFileSync('./example-pattern.json', {
//       encoding: 'utf-8',
//     });
...
const main = async () => {
    try {
        ...
        const forkOwner = "floresmaya/serverless-patterns"
        const forkRepo = "flormaya-feature-amplify-static-source"
        const forkURL = `https://github.com/${forkOwner}/tree/${forkRepo}`;

        console.log(`https://beta.serverlessland.com/patterns/sandbox?repo=${encodeURIComponent(forkURL)}&pattern=${encodeURIComponent(JSON.stringify(parsedJSON))}`,)
    }
    catch (error) {
    console.info(error);
    }
