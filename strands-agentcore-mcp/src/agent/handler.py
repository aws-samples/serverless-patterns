...
# Environment variable for the Cognito User Pool issuer URL used for JWKS
# signature verification. Format:
#   https://cognito-idp.<region>.amazonaws.com/<user-pool-id>
JWT_ISSUER_ENV = "COGNITO_ISSUER"


def _extract_token(event: dict) -> str | None:
    """Extract JWT token from the Lambda event.

    Checks for the token in three locations:
    1. event["jwt"] — direct token field (used by scripts/test.sh payloads)
    2. event["token"] — alternate direct token field
    3. event["headers"]["Authorization"] — Bearer token from headers
    ...
    """
    token = event.get("jwt") or event.get("token")
    if token:
        return token
...
def lambda_handler(event: dict, context) -> dict:
    ...
    # Validate JWT and extract username. Use the Cognito issuer from the
    # environment when available so signatures are verified via JWKS.
    issuer = os.environ.get(JWT_ISSUER_ENV)
    try:
        claims = validate_token(token, issuer=issuer)
    ...
    # Also check nested body.prompt (from API Gateway-shaped payloads)
    if not prompt:
        body = event.get("body", {})
