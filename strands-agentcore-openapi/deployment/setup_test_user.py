...
def setup_test_user(
    ...
    """Set up test user. Password must be supplied — no default is provided."""
    if not password:
        print("ERROR: --password is required. Generate one with: python3 -c \"import secrets,string; print(''.join(secrets.choice(string.ascii_letters+string.digits+'!@#$%') for _ in range(16)))\"")
        return False
    print("=" * 60)
    ...
    # Save only non-sensitive stack outputs — do NOT persist password or tokens
    credentials = {
        'user_pool_id': user_pool_id,
        'client_id': client_id,
        'username': username,
        'email': email,
        # Password and tokens are intentionally not saved to disk
    }

    creds_file = Path("deployment/stack_outputs.json")
    ...
    print(f"\n  ✓ User details added to: {creds_file}")

    print("\n" + "=" * 60)
    print("✓ TEST USER READY")
    print("=" * 60)
    print(f"\nUsername: {username}")
    print(f"Password: (not saved — was supplied via --password flag)")
    print("\nTo test, run: ./scripts/test.sh (generated by deploy.sh)")
...
def main():
    ...
    parser.add_argument(
        '--password',
        required=True,
        help='Password for test user (required — no default)'
    )
