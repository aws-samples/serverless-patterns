...
def format_error_response(
    ...
    """Format error response for Lambda with appropriate HTTP status codes.
    
    This function ensures appropriate HTTP status codes are returned:
    - 400 for client errors (invalid input, missing parameters)
    - 401 for authentication errors (invalid JWT, missing JWT)
    - 500 for server errors (internal failures, service unavailable)
...
class ErrorHandler:
    """Centralized error handler for Lambda functions with appropriate HTTP status codes.
    
    This class ensures appropriate HTTP status codes are returned (Requirement 8.10):
    - 400 for client errors (invalid input, missing parameters)
    - 401 for authentication errors (invalid JWT, missing JWT)
    - 500 for server errors (internal failures, service unavailable)
    """
    ...
    def handle_authentication_error(error: Exception) -> dict:
        ...
        Returns 401 for authentication errors (Requirement 8.10).
        
        Args:
            error: Authentication error
            
        Returns:
            Lambda error response with 401 status
        """
    ...
    def handle_aws_error(error: Exception) -> dict:
        """Handle AWS service errors with appropriate status codes.
        
        Maps AWS errors to appropriate HTTP status codes (Requirement 8.10):
        - 400 for validation errors
        - 401 for authentication errors
        - 403 for authorization errors
        - 404 for not found errors
        - 429 for throttling errors
        - 500 for internal/service errors
        
        Args:
            error: AWS ClientError
            
        Returns:
            Lambda error response with appropriate status code
        """
        error_code = getattr(error, 'response', {}).get('Error', {}).get('Code', 'Unknown')
        message = get_user_friendly_message(error_code)
        
        # Map AWS errors to appropriate HTTP status codes (Requirement 8.10)
    ...
    def handle_validation_error(error: Exception) -> dict:
        ...
        Returns 400 for client errors (Requirement 8.10).
        
        Args:
            error: Validation error
            
        Returns:
            Lambda error response with 400 status
        """
        return format_error_response(
            400,
            str(error),
    ...
    @staticmethod
    def handle_missing_parameter_error(parameter_name: str) -> dict:
        """Handle missing parameter errors with 400 status code.
    ...
    def handle_generic_error(error: Exception) -> dict:
        """Handle generic errors with 500 status code.
        
        Returns 500 for server errors (Requirement 8.10).
        
        Args:
            error: Generic error
            
        Returns:
            Lambda error response with 500 status
        """
        return format_error_response(
            500,
